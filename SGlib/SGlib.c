...
/* PGTADDRESS and CGTADDRESS are defined in SGlib.h */
...
void SG_blankArea(unsigned char x1, unsigned char y1, unsigned char x2, unsigned char y2) {
    unsigned int n = (unsigned int)(x2 - x1 + 1) << 3;  /* width * 8 bytes per row */
    unsigned int address = PGTADDRESS + (((unsigned int)y1 << 8) | ((unsigned int)x1 << 3));

    for (unsigned char loc_y = y1; loc_y <= y2; ++loc_y) {
        SG_set_address_VRAM(address);
        for (unsigned int i = n; i != 0; --i)
            VDPDataPort = 0;
        address += SG_MAX_TILE_RES_X << 3;
    }
}
...
void SG_initBMPmode (unsigned char bg_color, unsigned char fg_color) {
  ...
  unsigned char tileno=0;

  //setup nametable
  address = PNTADDRESS;
  for (unsigned char y=0; y != SG_MAX_TILE_RES_Y; ++y) {
    for (unsigned char x=0; x != SG_MAX_TILE_RES_X; ++x) {
        SG_set_address_VRAM (address + x);
        SG_setTile(tileno++);
    }
    address += 1 << 5;
  }

  //setup foreground and background for whole screen
  address = CGTADDRESS;
  for (unsigned char y=0; y != SG_MAX_TILE_RES_Y; ++y) {
    SG_set_address_VRAM (address);
    for (unsigned char x=SG_MAX_TILE_RES_X; x != 0; --x) {
...
void SG_setPixel (unsigned char x, unsigned char y, unsigned char color) {
  ...
  // set color data — write-only (assumes bgcolor = SG_COLOR_TRANSPARENT = 0)
  if (!(color & NO_COLOR_UPDATE)) {
    color &= 0x0F;
    SG_set_address_VRAM (CGTADDRESS + address);
    VDPDataPort = fg_color ? (color << 4) : color;
  }
}
...
void SG_setPixelAddr (unsigned int pgt_addr, unsigned char mask, unsigned char color) {
  unsigned char data;
  _Bool fg_color = (color <= 0x10);

  SG_set_address_VRAM_read (pgt_addr);
  WAIT_VRAM;
  data = VDPDataPort;
  if (fg_color) data |= mask;
  else          data &= ~mask;
  SG_set_address_VRAM (pgt_addr);
  VDPDataPort = data;

  if (!(color & NO_COLOR_UPDATE)) {
    color &= 0x0F;
    SG_set_address_VRAM (CGTADDRESS + pgt_addr);
    VDPDataPort = fg_color ? (color << 4) : color;
  }
}

/* low level functions, just to be used for dirty tricks ;) */
unsigned char SG_readVRAMbyte (unsigned int address) {
  SG_set_address_VRAM_read (address);
  WAIT_VRAM;
  return VDPDataPort;
}

void SG_writeVRAMbyte (unsigned int address, unsigned char data) {
  SG_set_address_VRAM (address);
  VDPDataPort = data;
}
